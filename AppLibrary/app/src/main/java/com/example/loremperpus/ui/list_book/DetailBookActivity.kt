...
class DetailBookActivity : AppCompatActivity() {
    ...
    private var livestock= MutableLiveData<Int>()
    private val readstock: LiveData<Int> get() = livestock
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.addAddCart.setOnClickListener {
            ...
            readstock.observe(this){
                if (it==0){
                    IsStatus=true
                    toastWarning("Stock Book tidak tersedia")
                }
                else{
                    IsStatus=false
                }
        ...
        viewModel.getDetailBook(token,id).observe(this) {
            when (it.state) {
                State.SUCCESS -> {
                    ...
                    val stock=bookResponse?.stock
                    livestock.value=stock ?:0
                    binding.bookTitle.text=title
                    binding.bookAuthorTv.text=author
                    binding.bookDesc.text=desc
                    binding.bookStock.text=stock.toString()
