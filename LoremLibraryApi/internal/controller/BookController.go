...
const (
	sampulPath = "C:\\Users\\PC-33\\Documents\\UKK\\LoremLibrary\\public\\storage\\sampul\\"
)
...
// @Success 200 {object} response.ResponseDataSuccess
// @Failure 400 {object} response.ResponseError
// @Router /api/book/validateQR/{code} [get]
func QRValidationDetail(c *fiber.Ctx) error {
	codeParam := c.Params("code")

	var book models.Books
	if err := config.DB.First(&book, "code= ?", codeParam).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return c.Status(404).JSON(fiber.Map{"status": "false", "message": "Data tidak di temukan"})
		}
		return c.Status(500).JSON(fiber.Map{"status": "false", "message": err.Error()})
